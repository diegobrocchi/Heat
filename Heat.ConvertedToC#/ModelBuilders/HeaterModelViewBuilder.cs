...
namespace Heat
{
	...
	public class HeaterModelViewBuilder
	{
		...
		public CreateHeaterViewModel GetCreateHeaterViewModel(int thermalUnitID)
		{
			...
			result.ManifacturerList = _db.Manifacturers.OrderBy(m => m.Name).ToList().ToSelectListItems(m => m.Name, m => m.ID, "");
			result.FuelList = _db.Fuels.OrderBy(f => f.Name).ToList().ToSelectListItems(f => f.Name, f => f.ID, "");
			result.ModelList = _db.ManifacturerModels.Include(mm => mm.Manifacturer).OrderBy(mm => mm.Manifacturer.Name).ThenBy(mm => mm.Model).ToList().ToSelectListItems(x => x.Model, x => x.ID, "");
