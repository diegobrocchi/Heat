...
namespace Heat
{
    ...
    public class PlantModelViewBuilder
    {
        ...
        public AddContactPlantViewModel GetAddContactPlantViewModel(int plantID)
        {
            ...
            result.AddressTypeList = _db.AddressTypes.OrderBy(x => x.Description).ToSelectListItems(x => x.Description, x => x.ID.ToString(), "");
            result.RoleList = _db.PlantRoles.OrderBy(x => x.Role).ToSelectListItems(x => x.Role, x => x.ID.ToString(), "");
