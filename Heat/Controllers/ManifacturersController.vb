Imports System
Imports System.Collections.Generic
Imports System.Data
Imports System.Data.Entity
Imports System.Linq
Imports System.Net
Imports System.Web
Imports System.Web.Mvc
Imports Heat.Models
Imports Heat.Repositories
Imports Heat.ViewModels.ManifacturerModels

Namespace Controllers
    Public Class ManifacturersController
        Inherits System.Web.Mvc.Controller

        Dim db As New HeatDBContext

        ' GET: Manifacturers
        Function Index() As ActionResult
            Return View(db.Manifacturers.ToList())


        End Function

        ' GET: Manifacturers/Details/5
        Function Details(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim manifacturer As Manifacturer = db.Manifacturers.Find(id)
            If IsNothing(manifacturer) Then
                Return HttpNotFound()
            End If
            Return View(manifacturer)
        End Function

        ' GET: Manifacturers/Create
        Function Create() As ActionResult
            Return View()
        End Function

        ' POST: Manifacturers/Create
        'To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        'more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        <HttpPost()>
        <ValidateAntiForgeryToken()>
        Function Create(<Bind(Include:="ID,Name")> ByVal manifacturer As Manifacturer) As ActionResult
            If ModelState.IsValid Then
                db.Manifacturers.Add(manifacturer)
                db.SaveChanges()
                Return RedirectToAction("Index")
            End If
            Return View(manifacturer)
        End Function

        ' GET: Manifacturers/Edit/5
        Function Edit(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim manifacturer As Manifacturer = db.Manifacturers.Find(id)
            If IsNothing(manifacturer) Then
                Return HttpNotFound()
            End If
            Return View(manifacturer)
        End Function

        ' POST: Manifacturers/Edit/5
        'To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        'more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        <HttpPost()>
        <ValidateAntiForgeryToken()>
        Function Edit(<Bind(Include:="ID,Name")> ByVal manifacturer As Manifacturer) As ActionResult
            If ModelState.IsValid Then
                db.Entry(manifacturer).State = EntityState.Modified
                db.SaveChanges()
                Return RedirectToAction("Index")
            End If
            Return View(manifacturer)
        End Function

        ' GET: Manifacturers/Delete/5
        Function Delete(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim manifacturer As Manifacturer = db.Manifacturers.Find(id)
            If IsNothing(manifacturer) Then
                Return HttpNotFound()
            End If
            Return View(manifacturer)
        End Function

        ' POST: Manifacturers/Delete/5
        <HttpPost()>
        <ActionName("Delete")>
        <ValidateAntiForgeryToken()>
        Function DeleteConfirmed(ByVal id As Integer) As ActionResult
            Dim manifacturer As Manifacturer = db.Manifacturers.Find(id)
            db.Manifacturers.Remove(manifacturer)
            db.SaveChanges()
            Return RedirectToAction("Index")
        End Function

        Protected Overrides Sub Dispose(ByVal disposing As Boolean)
            If (disposing) Then
                db.Dispose()
            End If
            MyBase.Dispose(disposing)
        End Sub
    End Class
End Namespace
